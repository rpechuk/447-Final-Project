...
class SlangGenTrainer:
    ...
    def get_trained_embeddings(self, slang_ind, fold_name='default', model_path='SBERT_contrastive', encode_test=True):
        
        model_name = self.out_dir + '/' + fold_name + '/SBERT_data/' + model_path
        self.load_sense_encoder(model_name, model_path)
        
        return self.get_sense_embeddings(slang_ind, fold_name, encode_test=encode_test)
        
    def get_sense_embeddings(self, slang_ind, fold_name='default', encode_test=True):
        ...
        sense_embeds = self.sense_encoder.encode_dataset(self.dataset, slang_ind, encode_test=encode_test)
        if encode_test:
            ...
            np.savez(out_dir+"sum_embed_"+self.sense_encoder.name+".npz", train=sense_embeds['train'], dev=sense_embeds['dev'], standard=sense_embeds['standard'])
